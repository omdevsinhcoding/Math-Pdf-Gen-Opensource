...
# type3_app.py — Addition_type3 (Type-3: Make total = 100)
# Build (example):
#   pyinstaller --onefile --windowed --clean --name Addition_type3 --icon=logo.ico --add-data "logo.ico;." --add-data "logo.png;." type3_app.py
...
def _set_windows_appid(app_id="om.addition_type3"):
    try:
        import ctypes
...
def _get_cached_quotes():
    ...
    cache_dir = os.path.join(appdata, "Addition_type3")
...
BASE_NAME = "Addition_type3"
MAX_PDFS, MIN_PDFS = 4000, 1

TASK_PREFIX_FOLDER = "Addition_type3_Reminder_"
TASK_PREFIX_ZIP    = "Addition_type3_Reminder_ZIP_"
...
# -------------------- TYPE-3 QUESTION LOGIC --------------------
# Generate questions of the form: A + ____ = 100, answer = 100 - A
# We'll use A from 1..99 excluding 100; prefer 2-digit numbers but include variety.
def gen_qs(seed):
    r = random.Random(seed)
    pool = list(range(1,100))  # 1..99
    # Prefer two-digit by duplicating them
    pool = pool + list(range(11,100))
    r.shuffle(pool)
    data = []
    used = set()
    for a in pool:
        if a in used: 
            continue
        b = 100 - a
        # Avoid duplicates like (a=30,b=70) and (a=70,b=30) by locking on a only
        data.append((a, b))
        used.add(a)
        if len(data) >= 100:
            break
    # Guarantee 100 entries
    while len(data) < 100:
        a = r.randint(1,99)
        data.append((a, 100-a))
    # Shuffle for randomness
    r.shuffle(data)
    # Map to (question parts for print; answer for key)
    out = []
    for a,b in data[:100]:
        out.append((a, b))  # b is the missing number / also the answer
    return out
...
def draw_header_var(c, t, idx):
    title = "Type-3 • Make 100 (Find the missing addend)"; sub="100 Questions"
    rng = random.Random(300_000 + idx * 73)
...
def draw_answers_text(c,ax,ay,aw,ah,data):
    ...
    for i in range(100):
        cx,rr=i%cols, i//cols
        c.drawString(ax0+cx*cw, ay0-rr*12, f"{i+1:>2}. {data[i][1]}")
...
def draw_questions(c,x,y,w,pill_y,pill_h,ans_ay,ans_ah,t,data):
    ...
    c.drawString(x+18, start_y+12, "Example: 49 + ____ = 100  →  Answer = 51")
    line_h=17.5
    for i in range(100):
        ...
        a,b=data[i]
        c.drawString(cx-col_w/2+32, base_y, f"{a} + ____ = 100")

def generate_pdfs(target, total, progress_cb=None):
    os.makedirs(target, exist_ok=True)
    for idx in range(total):
        if progress_cb: progress_cb(idx)
        t=palette(idx); data=gen_qs(9000+idx*53)
        # ensure we always have 100 entries
        if len(data) < 100:
            # pad safely
...
# -------------------- Reminder system (same as others) --------------------
def _is_bundled():
...
def _list_our_task_rows():
    ...
    for row in reader:
        ...
        if (tn.startswith(TASK_PREFIX_FOLDER) or tn.startswith(TASK_PREFIX_ZIP)
            or tn.startswith("Type3_Reminder_") or tn.startswith("Type3_Zip_Reminder_")
...
def remove_all_reminders():
    for tn in _list_our_task_names():
        try:
            ...
            pass
    for i in range(1,10):
        for base in (TASK_PREFIX_FOLDER,TASK_PREFIX_ZIP,"Type3_Reminder_","Type3_Zip_Reminder_"):
            try:
...
def _open_folder_interactive(path):
    ...
    quote = _pick_quote()
    dlg = ReminderPopup(root, "Addition_type3 Reminder",
    ...
    def _do():
        if path and os.path.isdir(path):
            ...
            newdir = fd.askdirectory(title="Pick your Addition_type3 PDFs folder")
...
def _open_zip_interactive(zipf):
    root=tk.Tk(); root.withdraw()
    quote = _pick_quote()
    dlg = ReminderPopup(root, "Addition_type3 Reminder (ZIP)",
    ...
    def _do():
        if zipf and os.path.isfile(zipf):
            ...
            folder = fd.askdirectory(title="Pick folder that contains your Addition_type3 ZIP")
...
class AppUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Addition_type3 Generator (Type-3)"); self.resizable(False, False)
        safe_icon(self)
    ...
    def remove_reminders(self):
        ...
        messagebox.showinfo("Reminders", "All Addition_type3 reminders removed.")
        self._next_run=None
        self.countdown.grid_remove()
