...
# type2_app.py — Addition_type2 (Type-2: Break First Number into Tens + Ones)
# Build (PowerShell one-liner):
# pyinstaller --onefile --windowed --clean --name Addition_type2 --icon=logo.ico --add-data "logo.ico;." --add-data "logo.png;." type2_app.py
...
def _set_windows_appid(app_id="om.addition_type2"):
    try:
        import ctypes
...
def _get_cached_quotes():
    ...
    cache_dir = os.path.join(appdata, "Addition_type2")
...
BASE_NAME = "Addition_type2"
MAX_PDFS, MIN_PDFS = 4000, 1

TASK_PREFIX_FOLDER = "Addition_type2_Reminder_"
TASK_PREFIX_ZIP    = "Addition_type2_Reminder_ZIP_"
...
# -------------------- TYPE-2 QUESTION LOGIC --------------------
# Rule: first number is any 2-digit (not multiple of 10), second number is a multiple of 10.
# Method taught: break first number into tens+ones (e.g., 68 + 70 => 60+8 + 70 = 130 + 8 = 138).
def gen_qs(seed):
    r = random.Random(seed)
    tens_choices = [20,30,40,50,60,70,80,90]
    data = []
    for _ in range(200):
        a = r.randint(11,99)
        if a % 10 == 0:  # avoid pure tens to force 'break' thinking
            a += 1
            if a > 99: a -= 2
        b = r.choice(tens_choices)
        # Avoid too small/too large sums if you want; otherwise it's fine
        s = a + b
        data.append((a, b, s))
        if len(data) >= 100:
            break
...
def draw_header_var(c, t, idx):
    title = "Type-2 • Break Tens & Ones"; sub="100 Questions"
    rng = random.Random(210_000 + idx * 73)
...
def draw_questions(c,x,y,w,pill_y,pill_h,ans_ay,ans_ah,t,data):
    ...
    c.drawString(x+18, start_y+12, "Example: 68 + 70 = (60+8)+70 = 130 + 8 = 138")
    line_h=17.5
    for i in range(100):
...
def generate_pdfs(target, total, progress_cb=None):
    ...
    for idx in range(total):
        ...
        t=palette(idx); data=gen_qs(7100+idx*53)
...
def _list_our_task_rows():
    ...
    for row in reader:
        ...
        if (tn.startswith(TASK_PREFIX_FOLDER) or tn.startswith(TASK_PREFIX_ZIP)
            or tn.startswith("Type2_Reminder_") or tn.startswith("Type2_Zip_Reminder_")
...
def remove_all_reminders():
    for tn in _list_our_task_names():
        try:
            ...
            pass
    for i in range(1,10):
        for base in (TASK_PREFIX_FOLDER,TASK_PREFIX_ZIP,"Type2_Reminder_","Type2_Zip_Reminder_"):
            try:
...
def _open_folder_interactive(path):
    ...
    quote = _pick_quote()
    dlg = ReminderPopup(root, "Addition_type2 Reminder",
    ...
    def _do():
        if path and os.path.isdir(path):
            ...
            newdir = fd.askdirectory(title="Pick your Addition_type2 PDFs folder")
...
def _open_zip_interactive(zipf):
    root=tk.Tk(); root.withdraw()
    quote = _pick_quote()
    dlg = ReminderPopup(root, "Addition_type2 Reminder (ZIP)",
    ...
    def _do():
        if zipf and os.path.isfile(zipf):
            ...
            folder = fd.askdirectory(title="Pick folder that contains your Addition_type2 ZIP")
...
class AppUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Addition_type2 Generator (Type-2)"); self.resizable(False, False)
        safe_icon(self)
    ...
    def remove_reminders(self):
        ...
        messagebox.showinfo("Reminders", "All Addition_type2 reminders removed.")
        self._next_run=None
        self.countdown.grid_remove()
