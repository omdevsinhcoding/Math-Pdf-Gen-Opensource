...
# type4_from_type1_app.py — Addition_type4 (Type-4: Make Sum = 50)
# Built directly from your Type-1 template (UI, reminders, design are the same).
#
# Build (PowerShell one-liner):
#   pyinstaller --onefile --windowed --clean --name Addition_type4 ^
#     --icon=logo.ico --add-data "logo.ico;." --add-data "logo.png;." ^
#     type4_from_type1_app.py
...
def _set_windows_appid(app_id="om.addition_type4"):
    try:
        import ctypes
...
def _get_cached_quotes():
    ...
    cache_dir = os.path.join(appdata, "Addition_type4")
...
BASE_NAME = "Addition_type4"
MAX_PDFS, MIN_PDFS = 4000, 1

TASK_PREFIX_FOLDER = "Addition_type4_Reminder_"
TASK_PREFIX_ZIP    = "Addition_type4_Reminder_ZIP_"
...
def draw_header_var(c, t, idx):
    title = "Type-4 • Make Sum = 50"; sub="100 Questions"
...
def draw_questions(c,x,y,w,pill_y,pill_h,ans_ay,ans_ah,t,data):
    ...
    c.drawString(x+18, start_y+12, "Example: 27 + 23 = 50  •  Practice: a + ____ = 50")
    line_h=17.5
    for i in range(100):
        ...
        a,_=data[i]
        c.drawString(cx-col_w/2+32, base_y, f"{a} + ____ = 50")

# Generate 100 questions: a in 1..49
def gen_qs(seed):
    r = random.Random(seed)
    data = [(r.randint(1,49), 50) for _ in range(100)]
    return data
...
def _list_our_task_rows():
    ...
    for row in reader:
        ...
        if (tn.startswith(TASK_PREFIX_FOLDER) or tn.startswith(TASK_PREFIX_ZIP)
            or tn.startswith("Type4_Reminder_") or tn.startswith("Type4_Zip_Reminder_")
...
def remove_all_reminders():
    for tn in _list_our_task_names():
        try:
            ...
            pass
    for i in range(1,10):
        for base in (TASK_PREFIX_FOLDER,TASK_PREFIX_ZIP,"Type4_Reminder_","Type4_Zip_Reminder_"):
            try:
                _run_silent(["schtasks","/Delete","/TN",f"{base}{i}","/F"], check=True)
            except subprocess.CalledProcessError:
                pass

# -------------------- Custom Reminder Popup & actions (unchanged) --------------------
...
def _open_folder_interactive(path):
    ...
    quote = _pick_quote()
    dlg = ReminderPopup(root, "Addition_type4 Reminder",
    ...
    def _do():
        if path and os.path.isdir(path):
            ...
            newdir = fd.askdirectory(title="Pick your Addition_type4 PDFs folder")
...
def _open_zip_interactive(zipf):
    root=tk.Tk(); root.withdraw()
    quote = _pick_quote()
    dlg = ReminderPopup(root, "Addition_type4 Reminder (ZIP)",
    ...
    def _do():
        if zipf and os.path.isfile(zipf):
            ...
            folder = fd.askdirectory(title="Pick folder that contains your Addition_type4 ZIP")
            if folder: os.startfile(folder)
        dlg.destroy(); root.destroy()
    dlg.ok.configure(command=_do)
    dlg.wait_window(dlg)

# -------------------- Digital Clock Picker / Reminder Dialog (unchanged) --------------------
...
class ReminderDialog(tk.Toplevel):
    def __init__(self, parent, open_target, is_zip=False):
        ...
        self.big_clock = ClockPicker(frm, big=True); self.big_clock.grid(row=row,column=0,columnspan=2,sticky="w",pady=(0,8)); row+=1
...
class AppUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Addition_type4 Generator (Type-4)"); self.resizable(False, False)
        safe_icon(self)
    ...
    def remove_reminders(self):
        ...
        messagebox.showinfo("Reminders", "All Addition_type4 reminders removed.")
        self._next_run=None
        self.countdown.grid_remove()
